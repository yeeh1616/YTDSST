...
{
    public partial class FrmPopPaySuccessPartialFailure : Form
    {
        private WeXinSocketClient.RemoteCommandDelegate rdelegate;
        private bool is_can_countdown = false;
        int countdown_s = 60;

        public FrmPopPaySuccessPartialFailure()
        {
            InitializeComponent();
        }

        private void FrmPopPaySuccessPartialFailure_Load(object sender, EventArgs e)
        {
            Service.Context.ApplicationContext.getInstance().IS_HAS_POPUP_FORM = true;//用来作为是否情况购物车的条件之一

            this.lbSuccesNum.Text = String.Format("成功出票{0}张，",UserContext.getInstance().UserOrder.OutSuccNum);
            this.lbFailNum.Text = String.Format("失败{0}张", UserContext.getInstance().UserOrder.OutErrorNum);
        ...
        private void FrmPopPaySuccessPartialFailure_FormClosed(object sender, FormClosedEventArgs e)
        {
